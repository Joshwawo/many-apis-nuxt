<template>
     ...
     <NForm :rules="RULES">
       <NSpace vertical>
         <NFormItem path="voice" label="Voice actor">
           <NSelect v-model:value="selectedVoice" :options="voicesList"  placeholder="Selecciona una voz" filterable/>
         </NFormItem>
        </NSpace>

        <NSpace>
          <NFormItem label="Change language">
            <NSelect v-model:value="langSelected" :options="langList" @update:value="getVoices" placeholder="Selecciona una voz" filterable/>
          </NFormItem>
         ...
         <p class="text-gray-400/90 inline-block px-2">To ensure processing accuracy, we automatically remove any special characters or accents from the entered text. This means that only letters and numbers will be preserved in the final result. This function is used to prevent errors in text processing by the model.</p>
         <NButton class="mt-5  w-full text-center text-white" type="primary" @click="voiceGeneratedFn" :disabled="loading">Enviar</NButton>
    ...
    <template v-if="voiceGenerate.length > 0">
      <div class=" mt-10 text-white capitalize" v-for="(voice, index) in voiceGenerate" :key="index">
